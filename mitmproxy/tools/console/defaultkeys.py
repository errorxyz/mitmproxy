...
def map(km: Keymap) -> None:
    ...
    km.add("z", "console.command.confirm \"Clear flow list\" view.flows.remove @all", ["flowlist"], "Clear flow list")
    km.add(
        "Z", "console.command.confirm \"Purge all flows not showing\" view.flows.remove @hidden", ["flowlist"], "Purge all flows not showing"
    )
    km.add(
        "|",
    ...
    km.add("z", "console.command.confirm \"Clear eventlog\" eventstore.clear", ["eventlog"], "Clear")
